...
print(
    f"  • Encoded {basket_df.shape[0]} rows × {basket_df.shape[1]} items "
    f"in {time.time() - start:.1f}s"
)

# 4) Apriori
print("📈 Mining frequent itemsets (min_support=0.03)...")
start = time.time()
...
print(
    f"  • Found {len(freq_items)} frequent itemsets "
    f"in {time.time() - start:.1f}s"
)
...
print(
    ...
    "data/frequent_itemsets.csv and "
    "data/association_rules.csv"
)
